...
def sprout_leaves(t, leaves):
    ...
    return tree(label(t), [sprout_leaves(v, leaves) for v in branches(t)])
...
def add_trees(t1, t2):
    ...
    if (len1 == len2):
        return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip(branches(t1), branches(t2))])
    elif (len1 < len2):
        return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip(branches(t1), branches(t2))] + branches(t2)[len1:])
    else:
