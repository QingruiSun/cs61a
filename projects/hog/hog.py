...
def roll_dice(num_rolls, dice=six_sided):
    ...
    score_summation = 0
    k = 0
    score_one_flag = False
    while k < num_rolls:
        score_number = dice()
        if score_number == 1:
            score_one_flag = True
        score_summation = score_summation + score_number
        k = k + 1
    if score_one_flag == True:
        score_summation = 1
...
def piggy_points(score):
    ...
    if score == 0:
        return 3
    score_square_value = score * score
    min_digit = 9
    while score_square_value > 0:
        digit_value = score_square_value % 10
        if digit_value < min_digit:
            min_digit = digit_value
        score_square_value = score_square_value // 10
    return min_digit + 3
    # END PROBLEM 2
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    total_score = 0
    if num_rolls == 0:
        total_score = piggy_points(opponent_score)
        return total_score
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_digits = find_leftmost_digits(player_score)
    opponent_digits = find_leftmost_digits(opponent_score)
    if (player_digits[0] < opponent_digits[0]) and (player_digits[1] < opponent_digits[1]):
        return True
    return False
    # END PROBLEM 4

def find_leftmost_digits(score_number):
    """Find the left most two digits of the number.
       When the bit of the number is less than 2, add the 0 to the left bit.
    """
    if score_number < 10:
        return (0, score_number)
    while score_number >= 100:
        score_number = score_number // 10
    second_digit = score_number % 10
    first_digit = score_number // 10
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    averaged_dice = make_averaged(roll_dice, 1000)
    while i <= 10:
        averaged_value = averaged_dice(i, dice)
        if averaged_value > max_score:
            max_num = i
            max_score = averaged_value
        i = i + 1
    return max_num
